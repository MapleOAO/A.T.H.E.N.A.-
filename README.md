...
- 319 条关系：279 条已核验、40 条待核验；82 个资料来源。
- 172 条中文术语：118 条官方用字已核对、54 条工作译名待审。53 位英雄的中文名称均有国服目录依据。
- 搜索、筛选、一跳聚焦、全图缩放与平移、实体和关系详情、证据定位、术语搜索、收录进度、JSON 下载。
- 全部实现无第三方运行时依赖；43 项自动化测试。
- 84 项官方图像（53英雄头像、17剧情配图、2组织标志、12漫画局部），图谱、列表、详情与图像目录共用；88项名称占位明确标记待补。每张图像保留出处、说明与展示范围。
...
- `dist/preview.html`：单文件离线审阅版，内嵌数据、样式和原创脚本，下载后可直接在浏览器打开；12张漫画局部已内嵌；其余官方外链图像需联网，失败时回退名称图标。
