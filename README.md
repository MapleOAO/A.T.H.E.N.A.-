...
- 176 个实体：固定 Atlas 版本中的 53 位英雄、76 个其他角色、41 个组织，共 170 项完整目录；另有 Talon、一条组织异拼记录及4位从官方短篇新增的桥元长老。
- 57 / 57 条 Atlas 显式关联输入均已覆盖，保留原名、commit、blob 和 JSON pointer。Liao ↔ Echo 双向输入保留两个出处，图中去重。
- 360 条关系：321 条已核验、39 条待核验；93 个资料来源。
- 176 条中文术语：123 条官方用字已核对、53 条工作译名待审。53 位英雄的中文名称均有国服目录依据。
- 搜索、筛选、一跳聚焦、全图缩放与平移、实体和关系详情、证据定位、术语搜索、收录进度、JSON 下载。
- 全部实现无第三方运行时依赖；47 项自动化测试。
- 98 项官方图像（53英雄头像、19剧情配图、2组织标志、24原作局部），图谱、列表、详情与图像目录共用；78项名称占位明确标记待补。每张图像保留出处、说明与展示范围。
- 新增“黑爪之治”时期；旧身份与加入、离开、获救等后续事件分开保存。
- 本轮独立扩充4位桥元长老、4个官方中文名与8条关系；完善官方实体发现来源校验；研究记录先查 [研究索引](docs/research/README.md)。
...
- `dist/preview.html`：单文件离线审阅版，内嵌数据、样式和原创脚本，下载后可直接在浏览器打开；24张原作局部已内嵌；其余官方外链图像需联网，失败时回退名称图标。
