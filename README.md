...
- 361 条关系：322 条已核验、39 条待核验；96 个资料来源。
...
- 99 项官方图像（53英雄头像、19剧情配图、2组织标志、25原作局部），图谱、列表、详情与图像目录共用；77项名称占位明确标记待补。每张图像保留出处、说明与展示范围。
- 新增“黑爪之治”时期；旧身份与加入、离开、获救等后续事件分开保存。
- 本轮补入Sven官方漫画头像，新增托比昂保护堡垒事件，并为3条已有关系补证；研究记录先查 [研究索引](docs/research/README.md)。
...
- `dist/preview.html`：单文件离线审阅版，内嵌数据、样式和原创脚本，下载后可直接在浏览器打开；25张原作局部已内嵌；其余官方外链图像需联网，失败时回退名称图标。
