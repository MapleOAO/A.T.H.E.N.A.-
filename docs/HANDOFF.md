...
## 已保存、待集成的新研究（2026-09-18）

优先读取 docs/research/2026-09-18-npc-media-kiriko.md 及同名JSON：3张NPC官方剧情配图（伊菲、鲍勃、米娜·廖）、3项术语修正、12条带章节的关系建议。未修改正式data、未增加线上统计。执行环境在编辑前断开，无法运行校验、测试与构建，因此未更新gh-pages。不要将此次研究保存说成网站已发布。组织Logo仍待补；配图需新增official-scene类型，不能冒称独立头像。恢复后按研究记录nextSteps接入。
